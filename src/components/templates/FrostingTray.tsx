...
export interface FrostingTrayOption {
  rows: number;
  cols: number;
  label: string;
}

export interface FrostingChallenge {
  rows: number;
  cols: number;
  target: number;
  equivLabel: string;
...
export interface FrostingTrayProps {
  /** Tray shape options offered during free-explore. */
  exploreTrays: FrostingTrayOption[];
  /** Which explore tray to start on (index). Defaults to 0. */
  exploreStartIdx?: number;
  /** Challenge bank — runner picks a random one without repeating. */
  challenges: FrostingChallenge[];
  /** Skill stamp label shown on completion. */
  skillLabel: string;
  /** Called when the learner moves on to the next scene. */
  onComplete: () => void;
  /** Speech component (defaults to Penny). */
...
/**
 * Generic "shade N out of total cells on a grid" interaction template.
 * Drives the bakery equivalent-fractions scene: learner picks a tray shape,
 * shades freely to see simplifications, then matches a target like "1/3 of 12".
 */
const FrostingTray = ({
  exploreTrays,
  exploreStartIdx = 0,
  ...
  const [exploreTrayIdx, setExploreTrayIdx] = useState(exploreStartIdx);
